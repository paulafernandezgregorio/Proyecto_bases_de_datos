...
CREATE TABLE core.movie_cast (
    ...
    cast_id text,
    FOREIGN KEY (movie_id) REFERENCES core.movies (movie_id),
    FOREIGN KEY (actor_id) REFERENCES core.persons (person_id),
...
WHERE movie_id IS NOT NULL AND movie_id <> '';

INSERT INTO core.persons (person_id, gender, name)
SELECT DISTINCT actor_id, gender, name
...
WHERE rk.movie_id IS NOT NULL AND rk.movie_id <> ''
  AND rk.keyword_id IS NOT NULL AND rk.keyword_id <> '';
...
WHERE rg.movie_id IS NOT NULL AND rg.movie_id <> ''
  AND rg.genre_id IS NOT NULL AND rg.genre_id <> '';
...
WHERE rc.movie_id IS NOT NULL AND rc.movie_id <> ''
  AND rc.company_id IS NOT NULL AND rc.company_id <> '';
...
INSERT INTO core.movie_cast (movie_id, actor_id, character, cast_id)
SELECT
    cc.movie_id,
    cc.actor_id,
...
JOIN core.persons p ON p.person_id = cc.actor_id
WHERE cc.movie_id IS NOT NULL AND cc.movie_id <> ''
  AND cc.actor_id IS NOT NULL AND cc.actor_id <> ''
GROUP BY cc.movie_id, cc.actor_id;
...
WHERE cw.movie_id IS NOT NULL AND cw.movie_id <> ''
  AND cw.person_id IS NOT NULL AND cw.person_id <> ''
